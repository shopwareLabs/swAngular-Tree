angular.module('swAngularTree', [])
    .directive('swAngularTree', function ($sce, $http, $compile) {
        ...
        return {
            ...
            template: [
                '<div>',
                '    <div data-ng-repeat="node in list" data-ng-show="!node.parent.collapsed || options.noCollapse" data-ng-class="{bold: (node == currentNode)}">',
                '        <div data-ng-show="space" data-ng-repeat="space in node.spaces" data-ng-style="style.space">&nbsp;</div>',
                '        <span data-ng-style="style.node" data-ng-show="node.children && node.children.length > 0">',
                '            <span data-ng-click="toggle(node, !node.collapsed)" data-ng-show="node.collapsed && !options.noCollapse" class="glyphicon glyphicon-{{node.item.gridIcon_collapsed || node.item.gridIcon || options.collapsedNodeIcon || options.nodeIcon || \'folder-close\'}}"></span>',
                '            <span data-ng-click="toggle(node, !node.collapsed)" data-ng-show="!node.collapsed || options.noCollapse" class="glyphicon glyphicon-{{node.item.gridIcon_uncollapsed || node.item.gridIcon || options.uncollapsedNodeIcon || options.nodeIcon || \'folder-open\'}}"></span>',
                '            <input type="checkbox" data-ng-show="options.showCheckbox" data-ng-model="node.checked" data-ng-click="checkboxClick(node, node.checked!=true)" data-ng-disabled="(node.checked && !options.enableChecked) || (!node.checked && !options.enableUnchecked)"/>',
                '            <span data-ng-click="toggle(node, !node.collapsed)">{{node[\'label\']}}</span>',
                '        </span>',
                '        <span data-ng-style="style.node" data-ng-show="!node.children || node.children.length < 1">',
                ...
                '            <input type="checkbox" data-ng-show="options.showCheckbox || options.showCheckboxOnLeaf" data-ng-model="node.checked" data-ng-click="checkboxClick(node, node.checked!=true)" data-ng-disabled="(node.checked && !options.enableChecked) || (!node.checked && !options.enableUnchecked)"/>',
